...
public class UserService implements IUserService {
    ...
    /**
     * Calls UserRepository's findByUsername method to retrieves the user with the username and checks if user
     * is not already registered, if so DataAlreadyRegisteredException is thrown. Else, encrypts the user password
     * by calling passwordEncoder's encode method, adds data of the given userDTO object to model object to save
     * it by calling UserRepository's save method and then converts the saved User model object to UserDTO object.
     *
    ...
    /**
     * Checks if the given user to delete is registered by calling UserRepository's findById method, if so user
     * found is deleted by calling UserRepository's delete method. Else, ResourceNotFoundException is thrown.
     *
    ...
    /**
     * Calls UserRepository's findById method to retrieves the user with the given id and checks if user exists
     * in database, if so converts the found User to UserDTO object. Else, ResourceNotFoundException is thrown.
     *
     ...
     * ResourceNotFoundException is thrown. Else, each user from the list is converted to an UserDTO object
